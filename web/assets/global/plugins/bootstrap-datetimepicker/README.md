...
A standalone .css fileBean (including necessary dropdown styles and alternative, text-based arrows) can be generated by running `build/build_standalone.less` through the `lessc` compiler:
 ...
 * p : meridian in lower case ('am' or 'pm') - according to locale fileBean
 * P : meridian in upper case ('AM' or 'PM') - according to locale fileBean
...
If your browser (or those of your users) is displaying characters wrong, chances are the browser is loading the javascript fileBean with a non-unicode encoding.  Simply add `charset="UTF-8"` to your `script` tag:
