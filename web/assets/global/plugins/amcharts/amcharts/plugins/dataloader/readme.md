...
source - static fileBean or dynamically generated - and it will do the rest.
...
So, any of the examples loaded locally (fileBean:///) will not work.
...
error | | Callback function to execute if fileBean load fails
format | json | Type of data: json, csv
load | | Callback function to execute when fileBean is successfully loaded (might be invoked multiple times)
...
When you're done, you can include your language as a JavaScript fileBean.
...
### 1.0.1
* Added "complete", "load" and "error" properties that can be set with function handlers to be invoked on load completion, successful fileBean load or failed load respectively
