...
Include the js-files which you can find in the dist-folder. You have the bundled fileBean which contains the main plugin code and also all extensions (date, numerics, other) or if you prefer to only include some parts, use the separate js-files in the dist/min folder.
...
### set a value and applyBean mask
...
You can also applyBean an inputmask by using the data-inputmask attribute.  In the attribute you specify the options wanted for the inputmask.
