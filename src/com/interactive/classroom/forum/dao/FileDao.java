package com.interactive.classroom.forum.dao;
...
public class FileDao{
	 ...
	 * 功能：返回结果集
	 */
	public JSONObject getRecord(ForumFile query) throws SQLException, IOException, JSONException {
		String tableName="forum_file";
		//开始查询数据库
		String resultMsg="ok";
		int resultCode=0;
	...
	public JSONObject modifyRecord(ForumFile file) throws JSONException{
		...
		try {
			...
			//构造sql语句，根据传递过来的查询条件参数
			String tableName="forum_file";
			String sql="update "+tableName+" set title='"+file.getTitle()+"',content='"+file.getContent()+"',limit_time='"+file.getLimitTime()+"',status='"+file.getStatus()+"' where id="+file.getId();
	...
	public JSONObject addRecord(ForumFile file) throws JSONException, SQLException{
		String resultMsg="ok";
		int resultCode=0;
		List jsonList = new ArrayList();
		ylx_db query_db = new ylx_db(file.getDbName());
	...
	public JSONObject setRecordTop(String action,String dbName,String type,String userId,String id) throws JSONException, SQLException{
		...
		//构造sql语句，根据传递过来的查询条件参数
		String sql="select max(priority) as priority from forum_file where user_id='"+userId+"'";
		int priority=0;
		ResultSet rs=query_db.executeQuery(sql);
		if(rs.next()){
		...
		query_db.close();
		update_db.executeUpdate("update forum_file set priority="+(priority+1)+" where id="+id);
		update_db.close();
		//下面开始构建返回的json
		JSONObject jsonObj=new JSONObject();
		jsonObj.put("aaData",jsonList);
	 ...
	 */
	private String createGetRecordSql(ForumFile query){
		String sql="";
		String where="";
