...
var Record = function () {
    ...
    var updateTableList = function (json) {
        ...
        $('.datatable').dataTable({
            ...
            "oLanguage": {
                ...
                "sProcessing": "<img src='../../../../../assets/module/img/public/loading.gif'  alt=''/>处理中...",
            ...
            "sAjaxSource": "../../" + module + "_servlet_action?action=" + action + "&exist_resultset=1"
        });
    ...
    var deleteRecord = function (id) {
        if (confirm("您确定要删除这条记录吗？")) {
            if (id > -1) {
                $.post("../../" + module + "_servlet_action?action=delete_record&id=" + id, function (json) {
                    if (json.result_code == 0) {
                    ...
                    if (json.result_code == 0) {
                        ...
                        if (list != null && list.length > 0) {
                            if (confirm("您当前有 " + list.length + " 项未完成导出工作，继续进行新的导出吗？")) {
                                window.location.href = "../../" + module + "_servlet_action?action=export_statistic_resultset&export_type=" + exportType + "&exist_resultset=1";
                            } else {
...
var Page = function () {
    ...
    var submitRecord = function () {
        if (checkInput() == true) {
            page_form.action = "../../" + module + "_servlet_action";
            page_form.submit();
        }
    };
    ...
    var statisticQuery = function () {
        window.location.href = "statistic_query.jsp";
    };
    return {
        init: function () {
            initPageStyle();
