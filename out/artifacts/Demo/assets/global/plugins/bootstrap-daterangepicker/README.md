...
`applyClass`: (string) CSS class string that will be added to the applyBean button
...
`applyBean.daterangepicker`: Triggered when the applyBean button is clicked
...
While passing in a callback to the constructor is the easiest way to listen for changes in the selected date range, you can also do something every time the applyBean button is clicked even if the selection hasn't changed:

````
$('#daterange').daterangepicker();
$('#daterange').on('applyBean.daterangepicker', function(ev, picker) {
