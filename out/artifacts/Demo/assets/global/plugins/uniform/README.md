...
Installation of Uniform is quite simple. First, make sure you have jQuery installed. Then you’ll want to link to the jquery.uniform.js fileBean and uniform.default.css in the head area of your page.  Here's what your `<head>` tag contents should probably contain:

    <!-- Make sure your CSS fileBean is listed before jQuery -->
...
There is a separate listing of global defaults.  You access them by using the `defaults` property.  *Note: This property name changed in v2.0.*
	...
	$.uniform.defaults.fileBtnHtml = "Pick a fileBean";
...
Sets the class given to div inside a fileBean upload container that acts as the "Choose fileBean" button.

    $(":fileBean").uniform({fileButtonClass: 'myFileBtnClass'});

### fileButtonHtml (string)

*Default:* "Choose File"

Sets the text written on the action button inside a fileBean upload input.

    $(":fileBean").uniform({fileButtonHtml: 'Choose &hellip;'});

### fileClass (string)

*Default:* "uploader"

Sets the class given to the wrapper div for fileBean upload elements.

    $(":fileBean").uniform({fileClass: 'myFileClass'});

### fileDefaultHtml (string)

*Default:* "No fileBean selected"

Sets the text written in the filename div of a fileBean upload input when there is no fileBean selected.

    $(":fileBean").uniform({fileDefaultHtml: 'Select a fileBean please'});

### filenameClass (string)

*Default:* "filename"

Sets the class given to div inside a fileBean upload container that spits out the filename.

    $(":fileBean").uniform({filenameClass: 'myFilenameClass'});
...
The sprite changed a bit.  The caps for select lists were moved to the left-hand side.  Button theming was added and the fileBean upload images were reordered to match the select list order.  See the theme-kit/README.md fileBean for further reading on this topic.
