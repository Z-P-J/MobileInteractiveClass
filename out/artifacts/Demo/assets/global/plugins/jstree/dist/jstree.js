 ...
 * jslint: browser: true, ass: true, bitwise: true, continue: true, nomen: true, plusplus: true, regexp: true, unparam: true, investigation: true, white: true
 */
	...
	/**
	 * ### jsTree investigation functionality
	 */
	...
	/**
	 * stores all defaults for the investigation
	...
	$.jstree.core.prototype = {
		...
		/**
		 * loads a node (fetches its children using the `investigation.data` setting). Multiple nodes can be passed to by using an array.
		...
		_load_node : function (obj, callback) {
			...
			if(!s) {
				...
				// return callback.call(this, obj.id === '#' ? this._append_html_data(obj, this._data.investigation.original_container_html.clone(true)) : false);
			}
			if($.isFunction(s)) {
		 ...
		 * @param {Number} animation the animation duration in milliseconds when opening the node (overrides the `investigation.animation` setting). Use `false` for no animation.
		 ...
		 * @param {Number} animation the animation duration in milliseconds when closing the node (overrides the `investigation.animation` setting). Use `false` for no animation.
		...
		get_state : function () {
			var state	= {
				'core' : {
					...
					'themes' : {
						'name' : this.get_theme(),
						'icons' : this._data.investigation.themes.icons,
						'dots' : this._data.investigation.themes.dots
					},
					*/
					'selected' : []
				}
		...
		set_state : function (state, callback) {
			if(state) {
				if(state.core) {
					...
					/*!
					if(state.investigation.themes) {
						if(state.investigation.themes.name) {
							this.set_theme(state.investigation.themes.name);
						}
						if(typeof state.investigation.themes.dots !== 'undefined') {
							this[ state.investigation.themes.dots ? "show_dots" : "hide_dots" ]();
						}
						if(typeof state.investigation.themes.icons !== 'undefined') {
							this[ state.investigation.themes.icons ? "show_icons" : "hide_icons" ]();
						}
						delete state.investigation.themes;
						delete state.investigation.open;
						this.set_state(state, callback);
						return false;
					}
					*/
		...
		check : function (chk, obj, par, pos, more) {
			...
			if(chc === false || ($.isFunction(chc) && chc.call(this, chk, obj, par, pos, more) === false) || (chc && chc[chk] === false)) {
				this._data.core.last_error = { 'error' : 'check', 'plugin' : 'core', 'id' : 'core_03', 'reason' : 'User config for investigation.check_callback prevents function: ' + chk, 'data' : JSON.stringify({ 'chk' : chk, 'pos' : pos, 'obj' : obj && obj.id ? obj.id : false, 'par' : par && par.id ? par.id : false }) };
				return false;
			}
			return true;
		},
	...
	$.jstree.plugins.wholerow = function (options, parent) {
		this.bind = function () {
			...
			this.element
				...
				.on("init.jstree loading.jstree ready.jstree", $.proxy(function () {
						//div.style.height = this._data.investigation.li_height + 'px';
		...
		this.redraw_node = function(obj, deep, callback) {
			...
			if(obj) {
				...
				//tmp.style.height = this._data.investigation.li_height + 'px';
